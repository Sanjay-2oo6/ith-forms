-- Migration 058: Ensure section_position is returned in get_form_responses_tabular RPC
-- Previous migration 032 was updated with section_position but may not be deployed
-- This is a standalone fix to ensure the function returns the correct data

DROP FUNCTION IF EXISTS public.get_form_responses_tabular(uuid, integer, integer, text, text, timestamptz, timestamptz) CASCADE;
  ...
  SELECT json_build_object(
    ...
    'questions', (
      SELECT json_agg(
        ...
        ORDER BY COALESCE(sec.position, 999), q.position
      )
...
SELECT 'Migration 058: Fixed question ordering in RPC with section_position' as status;
