...
- The builder uses EXPLICIT save: edits stay in local state until the admin
  clicks Save, which calls the `save_form_builder` RPC atomically
  (`SaveIndicator` states: dirty → saving → saved/error).
...
- Migrations have duplicate numbers (two 015/016/017/018 files). Canonical
  order and rationale: `docs/migrations.md`. Next free number: 024.
